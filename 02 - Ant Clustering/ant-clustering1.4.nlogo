...
to setup
  ...
  create-particles particle-count [ 
    ...
    set size 1   
    set taken false
    ; Auswahl der Farben für die unterschiedlichen Cluster
    set color (who mod cluster-count) * 10 + 5
  ]
...
to check-for-pile     
  check-for-wrong-pile 
  check-for-right-pile  
end

; überprüft, ob Ameise einen falschen Partikel in der Nähe ab, um sich ggf. davon abzuwenden
to check-for-wrong-pile
  ...
  if false-pile != nobody[
    face false-pile
    right 180
  ]
end

to check-for-right-pile
  let p-free one-of particles-here with [ taken = false ]
  if p-free != nobody and [who] of p-free mod cluster-count = [who] of ([other-end] of one-of my-out-links) mod cluster-count [
     ; Auf leeren Nachbarn ablegen
     let p one-of neighbors with [ count particles-here = 0 ]
     if p != nobody[
       move-to p
       ask [other-end] of one-of my-out-links [ set taken false ] 
...
to check-for-particle
  if count particles-here > 0 [
    ...
    ; Aufnahme des Partikels, sofern vorhanden, und Bewegung nach vorne
    if to-carry != nobody [
      create-link-to to-carry [ tie ]
      ask to-carry[ set taken true ]
      jump step-length
    ]
  ]
end
